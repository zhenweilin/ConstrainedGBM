...
void MetaInfo::SetInfoFromHost(Context const& ctx, StringView key, Json arr) {
  ...
  // float info
  // add tau, last_pred, previous_pred, attribute_weight1, attribute_weight2  
  ...
  if (key == "weight") {
  ...
  } else if (key == "class_weight"){
    this->class_weight = std::move(*t.Data());
  } else if (key == "tau"){
    this->tau = std::move(*t.Data());
  } else if (key == "last_pred"){
    this->last_pred = std::move(*t.Data());
  } else if (key == "previous_pred"){
    this->previous_pred = std::move(*t.Data());
  } else if (key == "attribute_weight1"){
    this->attribute_weight1 = std::move(*t.Data());
  } else if (key == "attribute_weight2"){
    this->attribute_weight2 = std::move(*t.Data());
    ...
    std::cout << "key: class_weight" << key << std::endl;
