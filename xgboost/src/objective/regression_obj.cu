 ...
 * \brief Definition of single-value regression and classification objectives (add Neyman Pearson objective and Fairness objective).
 * \author Tianqi Chen, Kailong Chen, (Modified by Authorname)
 */
#include <dmlc/omp.h>
...
// additional include
#include "./regression_loss_np.h" 
#include "./regression_loss_fair.h"
    ...
    common::Transform<>::Init(
        [block_size, ndata, n_targets] XGBOOST_DEVICE(
          ...
          for (size_t idx = begin; idx < end; ++idx) {
            ...
            out_gpair_ptr[idx] = GradientPair(Loss::FirstOrderGradient(p, label, w) * w,
                                              Loss::SecondOrderGradient(p, label, w) * w);
          }
        },
...
template<typename Loss>
class RegLossNPObj : public ObjFunction {
 protected:
  HostDeviceVector<float> additional_input_;

 public:
  // 0 - label_correct flag, 1 - scale_pos_weight, 2 - is_null_weight
  RegLossNPObj(): additional_input_(3) {}
  ...
  void GetGradient(const HostDeviceVector<bst_float>& preds,
    ...
    common::Transform<>::Init(
        [block_size, ndata, n_targets] XGBOOST_DEVICE(
            ...
            common::Span<const bst_float> _preds,
            common::Span<const bst_float> _labels,
            common::Span<const bst_float> class_weight,
            common::Span<const bst_float> tau,
            common::Span<const bst_float> last_pred) {
          const bst_float* preds_ptr = _preds.data();
          const bst_float* labels_ptr = _labels.data();
          const bst_float* class_weight_ptr = class_weight.data();
          const bst_float* tau_ptr = tau.data();
          const bst_float* last_pred_ptr = last_pred.data();
          GradientPair* out_gpair_ptr = _out_gpair.data();
          const size_t begin = data_block_idx*block_size;
          const size_t end = std::min(ndata, begin + block_size);
          const float _scale_pos_weight = _additional_input[1];
          ...
          bst_float tau_val = tau_ptr[0];
          bst_float class_weight_val = class_weight_ptr[0];
          for (size_t idx = begin; idx < end; ++idx) {
            // bst_float p = Loss::PredTransform(preds_ptr[idx]);
            bst_float label = labels_ptr[idx];
            if (!Loss::CheckLabel(label)) {
            ...
            bst_float last_pred_val = last_pred_ptr[idx];
            out_gpair_ptr[idx] = GradientPair(
              Loss::FirstOrderGradient(preds_ptr[idx],
                                       label,
                                       class_weight_val,
                                       tau_val,
                                       last_pred_val),
              Loss::SecondOrderGradient(preds_ptr[idx],
                                        label,
                                        class_weight_val,
                                        tau_val,
                                        last_pred_val));
        ...
        common::Range{0, static_cast<int64_t>(n_data_blocks)}, nthreads, device)
        .Eval(&additional_input_, out_gpair, &preds, info.labels.Data(),
              &info.class_weight, &info.tau, &info.last_pred);
...
template<typename Loss>
class RegLossFairObj : public ObjFunction {
 protected:
  HostDeviceVector<float> additional_input_;

 public:
  // 0 - label_correct flag, 1 - scale_pos_weight, 2 - is_null_weight
  RegLossFairObj(): additional_input_(3) {}
  ...
  void GetGradient(const HostDeviceVector<bst_float>& preds,
    ...
    common::Transform<>::Init(
        [block_size, ndata, n_targets] XGBOOST_DEVICE(
            ...
            common::Span<const bst_float> _weights,
            common::Span<const bst_float> class_weight,
            common::Span<const bst_float> attribute_weight1,
            common::Span<const bst_float> attribute_weight2,
            common::Span<const bst_float> last_pred,
            common::Span<const bst_float> previous_pred,
            common::Span<const bst_float> tau) {
          const bst_float* preds_ptr = _preds.data();
          const bst_float* labels_ptr = _labels.data();
          const bst_float* weights_ptr = _weights.data();
          const bst_float* class_weight_ptr = class_weight.data();
          const bst_float* attribute_weight1_ptr = attribute_weight1.data();
          const bst_float* attribute_weight2_ptr = attribute_weight2.data();
          const bst_float* last_pred_ptr = last_pred.data();
          const bst_float* previous_pred_ptr = previous_pred.data();
          const bst_float* tau_ptr = tau.data();
          GradientPair* out_gpair_ptr = _out_gpair.data();
          const size_t begin = data_block_idx*block_size;
          const size_t end = std::min(ndata, begin + block_size);
          ...
          for (size_t idx = begin; idx < end; ++idx) {
            ...
            out_gpair_ptr[idx] = GradientPair(
              Loss::FirstOrderGradient(p,
                                       label,
                                       tau_ptr[0],
                                       class_weight_ptr[0],
                                       attribute_weight1_ptr[idx],
                                       attribute_weight2_ptr[0],
                                       last_pred_ptr[idx],
                                       previous_pred_ptr[idx],
                                       preds_ptr[idx],
                                       idx),
              Loss::SecondOrderGradient(p,
                                        label,
                                        tau_ptr[0],
                                        class_weight_ptr[0],
                                        attribute_weight1_ptr[idx],
        ...
        common::Range{0, static_cast<int64_t>(n_data_blocks)}, nthreads, device)
        .Eval(&additional_input_, out_gpair, &preds, info.labels.Data(), &info.weights_,
              &info.class_weight, &info.attribute_weight1, &info.attribute_weight2, &info.last_pred, &info.previous_pred, &info.tau);
...
XGBOOST_REGISTER_OBJECTIVE(LogisticNPRegression, LogisticNPRegression::Name())
.describe("Logistic regression for probability regression task (Neyman Pearson Classification).")
.set_body([]() { return new RegLossNPObj<LogisticNPRegression>(); });

XGBOOST_REGISTER_OBJECTIVE(LogisticNPClassification, LogisticNPClassification::Name())
.describe("Logistic regression for binary classification task (Neyman Pearson Classification).")
.set_body([]() { return new RegLossNPObj<LogisticNPClassification>(); });

// XGBOOST_REGISTER_OBJECTIVE(LogisticNPRaw, LogisticNPRaw::Name())
// .describe("Logistic regression for classification, output score "
//           "before logistic transformation (Neyman Pearson Classification).")
// .set_body([]() { return new RegLossNPObj<LogisticNPRaw>(); });


XGBOOST_REGISTER_OBJECTIVE(LogisticFairRegression, LogisticFairRegression::Name())
.describe("Logistic regression for probability regression task (Fairness).")
.set_body([]() { return new RegLossFairObj<LogisticFairRegression>(); });

XGBOOST_REGISTER_OBJECTIVE(LogisticFairClassification, LogisticFairClassification::Name())
.describe("Logistic regression for binary classification task (Fairness).")
.set_body([]() { return new RegLossFairObj<LogisticFairClassification>(); });

// XGBOOST_REGISTER_OBJECTIVE(LogisticFairRaw, LogisticFairRaw::Name())
// .describe("Logistic regression for classification, output score "
//           "before logistic transformation (Fairness).")
// .set_body([]() { return new RegLossFairObj<LogisticFairRaw>(); });
