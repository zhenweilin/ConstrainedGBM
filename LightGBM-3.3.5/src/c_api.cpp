...
class Booster {
 public:
  ...
  static void CheckDatasetResetConfig(
    ...
    if (new_param.count("tau_column") &&
        new_config.tau_column != old_config.tau_column) {
      Log::Fatal(
          "Cannot change tau_column after constructed Dataset handle.");
    }
    if (new_param.count("last_pred_column") &&
        new_config.last_pred_column != old_config.last_pred_column) {
      Log::Fatal(
          "Cannot change last_pred_column after constructed Dataset handle.");
    }
    if (new_param.count("previous_pred_column") &&
        new_config.previous_pred_column != old_config.previous_pred_column) {
      Log::Fatal(
          "Cannot change previous_pred_column after constructed Dataset handle.");
    }
    if (new_param.count("attribute_weight1_column") &&
        new_config.attribute_weight1_column != old_config.attribute_weight1_column) {
      Log::Fatal(
          "Cannot change attribute_weight1_column after constructed Dataset handle.");
    }
    if (new_param.count("attribute_weight2_column") &&
        new_config.attribute_weight2_column != old_config.attribute_weight2_column) {
      Log::Fatal(
          "Cannot change attribute_weight2_column after constructed Dataset handle.");
    }
