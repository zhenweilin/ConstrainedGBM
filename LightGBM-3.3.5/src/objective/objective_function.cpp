...
ObjectiveFunction* ObjectiveFunction::CreateObjectiveFunction(const std::string& type, const Config& config) {
  ...
  }else if (type == std::string("binary_np")) {
    return new BinaryLoglossNP(config);
  }else if (type == std::string("binary_fair")) {
    return new BinaryLoglossFair(config);
  ...
  } else if (type == std::string("multiclass_np")) {
    return new MulticlassSoftmaxNP(config);
  } else if (type == std::string("multiclass_fair")) {
    return new MulticlassSoftmaxFair(config);
  ...
  Log::Info("objective:%s", type.c_str());
...
ObjectiveFunction* ObjectiveFunction::CreateObjectiveFunction(const std::string& str) {
  ...
  if (type == std::string("regression")) {
    ...
    return new BinaryLoglossNP(strs);
  }else if (type == std::string("binary_fair")) {
    return new BinaryLoglossFair(strs);
  } else if (type == std::string("lambdarank")) {
    ...
    return new MulticlassSoftmaxNP(strs);
  } else if (type == std::string("multiclass_fair")) {
    return new MulticlassSoftmaxFair(strs);
  } else if (type == std::string("multiclassova")) {
