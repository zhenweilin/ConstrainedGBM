...
class MulticlassSoftmax: public ObjectiveFunction {
 public:
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    tau_ = metadata.tau();
    last_pred_ = metadata.last_pred();
    ini_score_ = metadata.init_score();
    previous_score_ = metadata.previous_score();
    previous_pred_ = metadata.previous_pred();
    attribute_weight1_ = metadata.attribute_weight1();
    attribute_weight2_ = metadata.attribute_weight2();
 ...
 private:
  ...
  const label_t* last_pred_;
  const label_t* previous_pred_;
  const label_t* tau_;
  const label_t* attribute_weight1_;
  const label_t* attribute_weight2_;
  const double_t* ini_score_;
  const double_t* previous_score_;
  std::vector<double> class_init_probs_;
};

class MulticlassSoftmaxNP: public ObjectiveFunction {
 public:
  explicit MulticlassSoftmaxNP(const Config& config) {
    num_class_ = config.num_class;
    // This factor is to rescale the redundant form of K-classification, to the non-redundant form.
    // In the traditional settings of K-classification, there is one redundant class, whose output is set to 0 (like the class 0 in binary classification).
    // This is from the Friedman GBDT paper.
  ...
  explicit MulticlassSoftmaxNP(const std::vector<std::string>& strs) {
    num_class_ = -1;
    for (auto str : strs) {
      auto tokens = Common::Split(str.c_str(), ':');
      if (tokens.size() == 2) {
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    tau_ = metadata.tau();
    // last_pred_ = metadata.last_pred();
    // Note: we use init_socre to store last_pred
    last_pred_ = metadata.init_score();
    previous_score_ = metadata.previous_score();
    previous_pred_ = metadata.previous_pred();
    attribute_weight1_ = metadata.attribute_weight1();
    attribute_weight2_ = metadata.attribute_weight2();
  ...
  void GetGradients(const double* score, score_t* gradients, score_t* hessians) const override {
    if (weights_ == nullptr) {
      std::vector<double> rec;
      std::vector<double> rec_copy; // add new value
      #pragma omp parallel for schedule(static) private(rec, rec_copy)
      for (data_size_t i = 0; i < num_data_; ++i) {
        rec.resize(num_class_);
        rec_copy.resize(num_class_);
        for (int k = 0; k < num_class_; ++k) {
          size_t idx = static_cast<size_t>(num_data_) * k + i;
          rec[k] = static_cast<double>(score[idx]);
        }
        ...
        for (int k = 0; k < num_class_; ++k) {
          auto p = rec[k];
          Log::Info("rec[k]:%d, rec_copy[k]:%d", rec[k], rec_copy[k]);
          size_t idx = static_cast<size_t>(num_data_) * k + i;
          if (label_int_[i] == k) {
            gradients[idx] = static_cast<score_t>(p - 1.0f + 1.0f / tau_[0] * (rec_copy[k] - last_pred_[idx]));
          } else {
            gradients[idx] = static_cast<score_t>(p  + 1.0f / tau_[0] * (rec_copy[k] - last_pred_[idx] ));
          }
          hessians[idx] = static_cast<score_t>(factor_ * (p * (1.0f - p) + 1.0f/tau_[0]));
        }
      }
    } else {
      std::vector<double> rec;
      std::vector<double> rec_copy; // add new value
      #pragma omp parallel for schedule(static) private(rec, rec_copy) // add rec_copy here or parallel
      for (data_size_t i = 0; i < num_data_; ++i) {
        rec.resize(num_class_);
        rec_copy.resize(num_class_);
        for (int k = 0; k < num_class_; ++k) {
        ...
        for (int k = 0; k < num_class_; ++k) {
          ...
          size_t idx = static_cast<size_t>(num_data_) * k + i;
          if (label_int_[i] == k) {
            // weights is lambda + w_k, corresponding class's `lambda + w_k`
            // i-th sample data
            gradients[idx] = static_cast<score_t>((p - 1.0f) * weights_[i] + (rec_copy[k] - last_pred_[idx])/tau_[0] );
          } else {
            gradients[idx] = static_cast<score_t>((p) * weights_[i] + (rec_copy[k] - last_pred_[idx]) / tau_[0]);
          }
          hessians[idx] = static_cast<score_t>((factor_ * p * (1.0f - p))* weights_[i] + 1.0f/tau_[0]);
        }
      }
    }
  }
 ...
 private:
  ...
  const label_t* attribute_weight1_;
  const label_t* attribute_weight2_;
  const double_t* last_pred_;
  const double_t* previous_score_;
  std::vector<double> class_init_probs_;
};


class MulticlassSoftmaxFair: public ObjectiveFunction {
 public:
  explicit MulticlassSoftmaxFair(const Config& config) {
    num_class_ = config.num_class;
    // This factor is to rescale the redundant form of K-classification, to the non-redundant form.
    // In the traditional settings of K-classification, there is one redundant class, whose output is set to 0 (like the class 0 in binary classification).
    // This is from the Friedman GBDT paper.
  ...
  explicit MulticlassSoftmaxFair(const std::vector<std::string>& strs) {
    num_class_ = -1;
    for (auto str : strs) {
      auto tokens = Common::Split(str.c_str(), ':');
      if (tokens.size() == 2) {
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    weights_ = metadata.weights();
    tau_ = metadata.tau();
    last_score_ = metadata.init_score();
    previous_score_ = metadata.previous_score();
    attribute_weight1_ = metadata.attribute_weight1();
    attribute_weight2_ = metadata.attribute_weight2();
    label_int_.resize(num_data_);
  ...
  void GetGradients(const double* score, score_t* gradients, score_t* hessians) const override {
    if (weights_ == nullptr) {
      ...
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        for (int k = 0; k < num_class_; ++k) {
          auto p = rec[k];
          auto temp01 = attribute_weight1_[i] * p;
          size_t idx = static_cast<size_t>(num_data_) * k + i;
          if (label_int_[i] == k) {
            gradients[idx] = static_cast<score_t>(temp01 + attribute_weight2_[0] * (rec_copy[k] - previous_score_[idx]) + 1.0f/tau_[0] * (rec_copy[k] - last_score_ [idx]) - attribute_weight1_[i]);
          } else {
            gradients[idx] = static_cast<score_t>(temp01 + attribute_weight2_[0] * (rec_copy[k] - previous_score_[idx]) + 1.0f/tau_[0] * (rec_copy[k] - last_score_ [idx]));
          }
          hessians[idx] = static_cast<score_t>((1.0f - p) * temp01 + 1.0f/tau_[0] + attribute_weight2_[0]);
        }
      }
    } else {
      std::vector<double> rec;
      ...
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        for (int k = 0; k < num_class_; ++k) {
          auto p = rec[k];
          auto temp01 = attribute_weight1_[i] * p * weights_[i];
          size_t idx = static_cast<size_t>(num_data_) * k + i;
          if (label_int_[i] == k) {
            gradients[idx] = static_cast<score_t>(temp01 + attribute_weight2_[0] * (rec_copy[k] - previous_score_[idx]) + 1.0f/tau_[0] * (rec_copy[k] - last_score_[idx]) - weights_[i] * attribute_weight1_[i]);
          } else {
            gradients[idx] = static_cast<score_t>(temp01 + attribute_weight2_[0] * (rec_copy[k] - previous_score_[idx]) + 1.0f/tau_[0] * (rec_copy[k] - last_score_[idx]));
          }
          hessians[idx] = static_cast<score_t>((1.0f - p)* temp01 + 1.0f/tau_[0] + attribute_weight2_[0]);
