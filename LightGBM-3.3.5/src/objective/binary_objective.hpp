...
class BinaryLoglossNP: public ObjectiveFunction {
 public:
  explicit BinaryLoglossNP(const Config& config,
                         std::function<bool(label_t)> is_pos = nullptr)
      : deterministic_(config.deterministic) {
    sigmoid_ = static_cast<double>(config.sigmoid);
    if (sigmoid_ <= 0.0) {
  ...
  explicit BinaryLoglossNP(const std::vector<std::string>& strs)
      : deterministic_(false) {
    sigmoid_ = -1;
    for (auto str : strs) {
      auto tokens = Common::Split(str.c_str(), ':');
  ...
  void GetGradients(const double* score, score_t* gradients, score_t* hessians) const override {
    ...
    if (weights_ == nullptr) {
      ...
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        const double response = -label * sigmoid_ / (1.0f + std::exp(label * sigmoid_ * score[i]));
        const double abs_response = fabs(response);
        gradients[i] = static_cast<score_t>(response + 1.0f/tau_[0] * (score[i] - last_score_[i]));
        hessians[i] = static_cast<score_t>(abs_response * (sigmoid_ - abs_response) + 1.0f/tau_[0]);
      }
    } else {
      #pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        const double abs_response = fabs(response);
        // weights_ is lambda
        gradients[i] = static_cast<score_t>(response * weights_[i] + 1.0f/tau_[0] * (score[i] - last_score_[i]));
        hessians[i] = static_cast<score_t>(abs_response * (sigmoid_ - abs_response) * weights_[i] + 1.0f/tau_[0]);
      }
    }
  }
...
class BinaryLoglossFair: public ObjectiveFunction {
 public:
  explicit BinaryLoglossFair(const Config& config,
                         std::function<bool(label_t)> is_pos = nullptr)
      : deterministic_(config.deterministic) {
    sigmoid_ = static_cast<double>(config.sigmoid);
    if (sigmoid_ <= 0.0) {
  ...
  explicit BinaryLoglossFair(const std::vector<std::string>& strs)
      : deterministic_(false) {
    sigmoid_ = -1;
    for (auto str : strs) {
      auto tokens = Common::Split(str.c_str(), ':');
  ...
  void GetGradients(const double* score, score_t* gradients, score_t* hessians) const override {
    ...
    if (weights_ == nullptr) {
      ...
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        gradients[i] = static_cast<score_t>(response * attribute_weight1_[i] + attribute_weight2_[0] * (score[i] - previous_score_[i]) + 1.0f/tau_[0] * (score[i] - last_score_[i]));
        hessians[i] = static_cast<score_t>(abs_response * (sigmoid_ - abs_response) * attribute_weight1_[i] + attribute_weight2_[0] + 1.0f/tau_[0]);
      }
    } else {
      #pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        const double response = -label * sigmoid_ / (1.0f + std::exp(label * sigmoid_ * score[i]));
        const double abs_response = fabs(response);
        gradients[i] = static_cast<score_t>(response * weights_[i] * attribute_weight1_[i]  + attribute_weight2_[0] * (score[i] - previous_score_[i]) + 1.0f/tau_[0] * (score[i] - last_score_[i]) );
        hessians[i] = static_cast<score_t>(abs_response * (sigmoid_ - abs_response) * weights_[i] * attribute_weight1_[i] + attribute_weight2_[0] + 1.0f/tau_[0]);
      }
    }
  }
