...
DatasetLoader::DatasetLoader(const Config& io_config, const PredictFunction& predict_fun, int num_class, const char* filename)
  ...
  last_pred_idx_ = NO_SPECIFIC;
  previous_pred_idx_ = NO_SPECIFIC;
  attribute_weight1_idx_ = NO_SPECIFIC;
  attribute_weight2_idx_ = NO_SPECIFIC;
  tau_idx_ = NO_SPECIFIC;
...
void DatasetLoader::SetHeader(const char* filename) {
  ...
  if (filename != nullptr && CheckCanLoadFromBin(filename) == "") {
    ...
    // load tau idx
    if (config_.tau_column.size() > 0) {
      if (Common::StartsWith(config_.tau_column, name_prefix)) {
        std::string name = config_.tau_column.substr(name_prefix.size());
        if (name2idx.count(name) > 0) {
          tau_idx_ = name2idx[name];
          Log::Info("Using column %s as tau", name.c_str());
        } else {
          Log::Fatal("Could not find tau column %s in data file", name.c_str());
        }
      } else {
        if (!Common::AtoiAndCheck(config_.tau_column.c_str(), &tau_idx_)) {
          Log::Fatal("tau_column is not a number,\n"
                     "if you want to use a column name,\n"
                     "please add the prefix \"name:\" to the column name");
        }
        Log::Info("Using column number %d as tau", tau_idx_);
      }
      ignore_features_.emplace(tau_idx_);
    }
    // load last_pred idx
    if (config_.last_pred_column.size() > 0) {
      if (Common::StartsWith(config_.last_pred_column, name_prefix)) {
        std::string name = config_.last_pred_column.substr(name_prefix.size());
        if (name2idx.count(name) > 0) {
          last_pred_idx_ = name2idx[name];
          Log::Info("Using column %s as last_pred", name.c_str());
        } else {
          Log::Fatal("Could not find last_pred column %s in data file", name.c_str());
        }
      } else {
        if (!Common::AtoiAndCheck(config_.last_pred_column.c_str(), &last_pred_idx_)) {
          Log::Fatal("last_pred_column is not a number,\n"
                     "if you want to use a column name,\n"
                     "please add the prefix \"name:\" to the column name");
        }
        Log::Info("Using column number %d as last_pred", last_pred_idx_);
      }
      ignore_features_.emplace(last_pred_idx_);
    }
    // load previous_pred idx
    if (config_.previous_pred_column.size() > 0) {
      if (Common::StartsWith(config_.previous_pred_column, name_prefix)) {
        std::string name = config_.previous_pred_column.substr(name_prefix.size());
        if (name2idx.count(name) > 0) {
          previous_pred_idx_ = name2idx[name];
          Log::Info("Using column %s as previous_pred", name.c_str());
        } else {
          Log::Fatal("Could not find previous_pred column %s in data file", name.c_str());
        }
      } else {
        if (!Common::AtoiAndCheck(config_.previous_pred_column.c_str(), &previous_pred_idx_)) {
          Log::Fatal("previous_pred_column is not a number,\n"
                     "if you want to use a column name,\n"
                     "please add the prefix \"name:\" to the column name");
        }
        Log::Info("Using column number %d as previous_pred", previous_pred_idx_);
      }
      ignore_features_.emplace(previous_pred_idx_);
    }
    // load attribute_weight1 idx
    if (config_.attribute_weight1_column.size() > 0) {
      if (Common::StartsWith(config_.attribute_weight1_column, name_prefix)) {
        std::string name = config_.attribute_weight1_column.substr(name_prefix.size());
        if (name2idx.count(name) > 0) {
          attribute_weight1_idx_ = name2idx[name];
          Log::Info("Using column %s as attribute_weight1", name.c_str());
        } else {
          Log::Fatal("Could not find attribute_weight1 column %s in data file", name.c_str());
        }
      } else {
        if (!Common::AtoiAndCheck(config_.attribute_weight1_column.c_str(), &attribute_weight1_idx_)) {
          Log::Fatal("attribute_weight1_column is not a number,\n"
                     "if you want to use a column name,\n"
                     "please add the prefix \"name:\" to the column name");
        }
        Log::Info("Using column number %d as attribute_weight1", attribute_weight1_idx_);
      }
      ignore_features_.emplace(attribute_weight1_idx_);
    }
    // load attribute_weight2 idx
    if (config_.attribute_weight2_column.size() > 0) {
      if (Common::StartsWith(config_.attribute_weight2_column, name_prefix)) {
        std::string name = config_.attribute_weight2_column.substr(name_prefix.size());
        if (name2idx.count(name) > 0) {
          attribute_weight2_idx_ = name2idx[name];
          Log::Info("Using column %s as attribute_weight2", name.c_str());
        } else {
          Log::Fatal("Could not find attribute_weight2 column %s in data file", name.c_str());
        }
      } else {
        if (!Common::AtoiAndCheck(config_.attribute_weight2_column.c_str(), &attribute_weight2_idx_)) {
          Log::Fatal("attribute_weight2_column is not a number,\n"
                     "if you want to use a column name,\n"
                     "please add the prefix \"name:\" to the column name");
        }
        Log::Info("Using column number %d as attribute_weight2", attribute_weight2_idx_);
      }
      ignore_features_.emplace(attribute_weight2_idx_);
    }
    // load group idx
...
Dataset* DatasetLoader::LoadFromFile(const char* filename, int rank, int num_machines) {
  ...
  if (bin_filename.size() == 0) {
    ...
    if (!config_.two_round) {
      ...
      // initialize label
      dataset->metadata_.Init(dataset->num_data_, weight_idx_, tau_idx_, last_pred_idx_, previous_pred_idx_, attribute_weight1_idx_, attribute_weight2_idx_, group_idx_);
      // extract features
...
void DatasetLoader::CheckDataset(const Dataset* dataset, bool is_load_from_binary) {
  ...
  if (is_load_from_binary) {
    ...
    if (config_.tau_column != "") {
      Log::Warning("Config tau_column works only in case of loading data directly from text file. It will be ignored when loading from binary file.");
    }
    if (config_.last_pred_column != "") {
      Log::Warning("Config last_pred_column works only in case of loading data directly from text file. It will be ignored when loading from binary file.");
    }
    if (config_.previous_pred_column != "") {
      Log::Warning("Config previous_pred_column works only in case of loading data directly from text file. It will be ignored when loading from binary file.");
    }
    if (config_.attribute_weight1_column != "") {
      Log::Warning("Config attribute_weight1_column works only in case of loading data directly from text file. It will be ignored when loading from binary file.");
    }
    if (config_.attribute_weight2_column != "") {
      Log::Warning("Config attribute_weight2_column works only in case of loading data directly from text file. It will be ignored when loading from binary file.");
    }
...
void DatasetLoader::ConstructBinMappersFromTextData(int rank, int num_machines,
  ...
  // check the range of label_idx, weight_idx, tau_idx, last_pred_idx, previous_pred_idx, attribute_weight1_idx, attribute_weight2_idx and group_idx
  CHECK(label_idx_ >= 0 && label_idx_ <= dataset->num_total_features_);
  CHECK(weight_idx_ < 0 || weight_idx_ < dataset->num_total_features_);
  CHECK(tau_idx_ < 0 || tau_idx_ < 1);
  CHECK(last_pred_idx_ < 0 || last_pred_idx_ < dataset->num_total_features_);
  CHECK(previous_pred_idx_ < 0 || previous_pred_idx_ < dataset->num_total_features_);
  CHECK(attribute_weight1_idx_ < 0 || attribute_weight1_idx_ < dataset->num_total_features_);
  CHECK(attribute_weight2_idx_ < 0 || attribute_weight2_idx_ < 1);
...
void DatasetLoader::ExtractFeaturesFromMemory(std::vector<std::string>* text_data, const Parser* parser, Dataset* dataset) {
  ...
  if (!predict_fun_) {
    ...
    for (data_size_t i = 0; i < dataset->num_data_; ++i) {
      ...
      for (auto& inner_data : oneline_features) {
        ...
        if (feature_idx >= 0) {
          ...
          }else if (inner_data.first == last_pred_idx_) {
            dataset->metadata_.SetLast_predAt(i, static_cast<label_t>(inner_data.second));
          }else if (inner_data.first == previous_pred_idx_) {
            dataset->metadata_.SetPrevious_predAt(i, static_cast<label_t>(inner_data.second));
          }else if (inner_data.first == attribute_weight1_idx_) {
            dataset->metadata_.SetAttribute_weight1At(i, static_cast<label_t>(inner_data.second));
          } else if (inner_data.first == group_idx_) {
            dataset->metadata_.SetQueryAt(i, static_cast<data_size_t>(inner_data.second));
          }
        }
      } // Need to modify if the length is not _num_data
      if (dataset->has_raw()) {
      ...
      oneline_features.clear();
      // parser
      parser->ParseOneLine(ref_text_data[0].c_str(), &oneline_features, &tmp_label);
      // free processed line:
      ref_text_data[0].clear();
      // shrink_to_fit will be very slow in linux, and seems not free memory, disable for now
      // text_reader_->Lines()[i].shrink_to_fit();
      std::vector<bool> is_feature_added(dataset->num_features_, false);
      // push data
      for (auto& inner_data : oneline_features) {
        ...
        if (feature_idx >= 0) {
          ...
          int group = dataset->feature2group_[feature_idx];
          int sub_feature = dataset->feature2subfeature_[feature_idx];
          dataset->feature_groups_[group]->PushData(tid, sub_feature, 0, inner_data.second);
          if (dataset->has_raw()) {
            feature_row[feature_idx] = static_cast<float>(inner_data.second);
          }
        } else {
          if (inner_data.first == tau_idx_) {
            dataset->metadata_.SetTauAt(0, static_cast<label_t>(inner_data.second));
          }else if (inner_data.first == attribute_weight2_idx_) {
            dataset->metadata_.SetAttribute_weight2At(0, static_cast<label_t>(inner_data.second));
          }
        }
      } // Need to modify if the length is not _num_data
      if (dataset->has_raw()) {
        for (size_t j = 0; j < feature_row.size(); ++j) {
          int feat_ind = dataset->numeric_feature_map_[j];
          if (feat_ind >= 0) {
            dataset->raw_data_[feat_ind][0] = feature_row[j];
          }
        }
      }
      dataset->FinishOneRow(tid, 0, is_feature_added);
      OMP_LOOP_EX_END();
    OMP_THROW_EX();
  } else {
    OMP_INIT_EX();
    ...
    std::vector<double> previous_score(dataset->num_data_ * num_class_);
    #pragma omp parallel for schedule(static) private(oneline_features) firstprivate(tmp_label, feature_row)
    for (data_size_t i = 0; i < dataset->num_data_; ++i) {
      OMP_LOOP_EX_BEGIN();
      const int tid = omp_get_thread_num();
      ...
      std::vector<double> oneline_previous_score(num_class_);
      predict_fun_(oneline_features, oneline_init_score.data());
      predict_fun_(oneline_features, oneline_previous_score.data());
      for (int k = 0; k < num_class_; ++k) {
        init_score[k * dataset->num_data_ + i] = static_cast<double>(oneline_init_score[k]);
        previous_score[k * dataset->num_data_ + i] = static_cast<double>(oneline_previous_score[k]);
      }
      // set label
      dataset->metadata_.SetLabelAt(i, static_cast<label_t>(tmp_label));
      // free processed line:
      ...
      predict_fun_(oneline_features, oneline_previous_score.data());
      for (int k = 0; k < num_class_; ++k) {
        init_score[k * dataset->num_data_ + 0] = static_cast<double>(oneline_init_score[k]);
        previous_score[k * dataset->num_data_ + 0] = static_cast<double>(oneline_previous_score[k]);
      }
      // free processed line:
      ref_text_data[0].clear();
      // shrink_to_fit will be very slow in Linux, and seems not free memory, disable for now
    ...
    // metadata_ will manage space of previous_score
    dataset->metadata_.SetInitScore(previous_score.data(), dataset->num_data_ * num_class_);
  }
  dataset->FinishLoad();
...
void DatasetLoader::ExtractFeaturesFromFile(const char* filename, const Parser* parser,
  ...
  if (predict_fun_) {
    ...
    previous_score = std::vector<double>(dataset->num_data_ * num_class_);
  }
  std::function<void(data_size_t, const std::vector<std::string>&)> process_fun =
    [this, &init_score, &previous_score, &parser, &dataset]
  (data_size_t start_idx, const std::vector<std::string>& lines) {
    ...
    for (data_size_t i = 0; i < static_cast<data_size_t>(lines.size()); ++i) {
      ...
      if (!previous_score.empty()) {
        ...
        predict_fun_(oneline_features, oneline_previous_score.data());
        for (int k = 0; k < num_class_; ++k) {
          previous_score[k * dataset->num_data_ + start_idx + i] = static_cast<double>(oneline_previous_score[k]);
        }
      }
      // set label
      ...
      for (auto& inner_data : oneline_features) {
        ...
        if (feature_idx >= 0) {
          ...
          if (inner_data.first == weight_idx_) {
            ...
            dataset->metadata_.SetLast_predAt(start_idx + i, static_cast<label_t>(inner_data.second));
          }else if (inner_data.first == previous_pred_idx_) {
            dataset->metadata_.SetPrevious_predAt(start_idx + i, static_cast<label_t>(inner_data.second));
          }else if (inner_data.first == attribute_weight1_idx_) {
            dataset->metadata_.SetAttribute_weight1At(start_idx + i, static_cast<label_t>(inner_data.second));
          } else if (inner_data.first == group_idx_) {
      ...
      oneline_features.clear();
      // parser
      parser->ParseOneLine(lines[0].c_str(), &oneline_features, &tmp_label);
      std::vector<bool> is_feature_added(dataset->num_features_, false);
      // push data
      for (auto& inner_data : oneline_features) {
        if (inner_data.first >= dataset->num_total_features_) { continue; }
        ...
        if (feature_idx >= 0) {
          ...
          int group = dataset->feature2group_[feature_idx];
          int sub_feature = dataset->feature2subfeature_[feature_idx];
          dataset->feature_groups_[group]->PushData(tid, sub_feature, start_idx + 0, inner_data.second);
          if (dataset->has_raw()) {
            feature_row[feature_idx] = static_cast<float>(inner_data.second);
          }
        } else {
          if (inner_data.first == tau_idx_) {
            dataset->metadata_.SetTauAt(start_idx + 0, static_cast<label_t>(inner_data.second));
          }else if (inner_data.first == attribute_weight2_idx_) {
            dataset->metadata_.SetAttribute_weight2At(start_idx + 0, static_cast<label_t>(inner_data.second));
          }
        }
      }
      if (dataset->has_raw()) {
