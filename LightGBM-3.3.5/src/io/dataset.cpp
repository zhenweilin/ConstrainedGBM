...
Dataset::Dataset(data_size_t num_data) {
  ...
  data_filename_ = "noname";
  num_data_ = num_data;
  metadata_.Init(num_data_, NO_SPECIFIC, NO_SPECIFIC, NO_SPECIFIC, NO_SPECIFIC, NO_SPECIFIC, NO_SPECIFIC, NO_SPECIFIC);
  is_finish_load_ = false;
...
#endif
  }else if (name == std::string("tau")){
#ifdef LABEL_T_USE_DOUBLE
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    metadata_.SetTau(field_data, num_element);
#endif
  }else if (name == std::string("last_pred")){
#ifdef LABEL_T_USE_DOUBLE
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    metadata_.SetLast_pred(field_data, num_element);
#endif
  }else if (name == std::string("previous_pred")){
#ifdef LABEL_T_USE_DOUBLE
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    metadata_.SetPrevious_pred(field_data, num_element);
#endif
  }else if (name == std::string("attribute_weight1")){
#ifdef LABEL_T_USE_DOUBLE
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    metadata_.SetAttribute_weight1(field_data, num_element);
#endif
  }else if (name == std::string("attribute_weight2")){
#ifdef LABEL_T_USE_DOUBLE
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    metadata_.SetAttribute_weight2(field_data, num_element);
#endif
  } else {
    return false;
  }
...
bool Dataset::SetDoubleField(const char* field_name, const double* field_data,
  ...
  }else if (name == std::string("previous_score")) {
    metadata_.SetPreviousScore(field_data, num_element);
  } else {
    return false;
  }
  return true;
...
bool Dataset::GetFloatField(const char* field_name, data_size_t* out_len,
  ...
  if (name == std::string("label") || name == std::string("target")) {
    ...
    *out_len = num_data_;
    ...
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    *out_ptr = metadata_.previous_pred();
    *out_len = num_data_;
#endif
  }else if (name == std::string("attribute_weight1")) {
#ifdef LABEL_T_USE_DOUBLE
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    *out_ptr = metadata_.attribute_weight1();
    *out_len = num_data_;
#endif
  }else if (name == std::string("attribute_weight2")) {
#ifdef LABEL_T_USE_DOUBLE
    Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#else
    *out_ptr = metadata_.attribute_weight2();
    *out_len = num_data_;
#endif
  } else {
    return false;
...
bool Dataset::GetDoubleField(const char* field_name, data_size_t* out_len,
  ...
  if (name == std::string("init_score")) {
    ...
    *out_ptr = metadata_.previous_score();
    *out_len = static_cast<data_size_t>(metadata_.num_previous_score());
  } else {
    return false;
  }
  return true;
