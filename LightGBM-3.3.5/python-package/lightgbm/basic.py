...
class _ConfigAliases:
    aliases = {"bin_construct_sample_cnt": {"bin_construct_sample_cnt",
               ...
               "weight_column": {"weight_column",
                                 "weight"},                
               "attribute_weight2_column": {"attribute_weight2_column",
                    "attribute_weight2"},
                "attribute_weight1_column": {"attribute_weight1_column",
                    "attribute_weight1"},
                "previous_pred_column": {"previous_pred_column",
                    "previous_pred"},
                "tau_column": {"tau_column",
                    "tau"},
                "last_pred_column": {"last_pred_column",
                    "last_pred"}}
...
FIELD_TYPE_MAPPER = {"label": C_API_DTYPE_FLOAT32,
                     ...
                     "last_pred": C_API_DTYPE_FLOAT32,
                     "tau": C_API_DTYPE_FLOAT32,
                     "previous_pred": C_API_DTYPE_FLOAT32,
                     "attribute_weight1": C_API_DTYPE_FLOAT32,
                     "attribute_weight2": C_API_DTYPE_FLOAT32,
                     "init_score": C_API_DTYPE_FLOAT64,
                     "previous_score": C_API_DTYPE_FLOAT64,
...
class Dataset:
    ...
    def __init__(self, data, label=None, reference=None,
                 weight=None, tau = None, last_pred = None, previous_pred = None,
                 attribute_weight1 = None, attribute_weight2 = None,
                 group=None, init_score=None, previous_score=None, silent='warn',
        ...
        previous_score : list, list of lists (for multi-class task), numpy array, pandas Series, pandas DataFrame (for multi-class task), or None, optional (default=None)
            Init score for Dataset.
    ...
    def get_params(self):
        ...
        if self.params is not None:
            ...
            dataset_params = _ConfigAliases.get("bin_construct_sample_cnt",
                                                ...
                                                "tau_column",
                                                "last_pred_column",
                                                "previous_pred_column",
                                                "attribute_weight1_column",
                                                "attribute_weight2_column",
                                                "zero_as_missing")
    ...
    def _set_previous_score_by_predictor(self, predictor, data, used_indices=None):
        data_has_header = False
        if isinstance(data, (str, Path)):
            # check data has header or not
            data_has_header = any(self.params.get(alias, False) for alias in _ConfigAliases.get("header"))
        num_data = self.num_data()
        if predictor is not None:
            previous_score = predictor.predict(data,
                                           raw_score=True,
                                           data_has_header=data_has_header,
                                           is_reshape=False)
            if used_indices is not None:
                assert not self.need_slice
                if isinstance(data, (str, Path)):
                    sub_previous_score = np.empty(num_data * predictor.num_class, dtype=np.float64)
                    assert num_data == len(used_indices)
                    for i in range(len(used_indices)):
                        for j in range(predictor.num_class):
                            sub_previous_score[i * predictor.num_class + j] = previous_score[used_indices[i] * predictor.num_class + j]
                    previous_score = sub_previous_score
            if predictor.num_class > 1:
                # need to regroup previous_score
                new_previous_score = np.empty(previous_score.size, dtype=np.float64)
                for i in range(num_data):
                    for j in range(predictor.num_class):
                        new_previous_score[j * num_data + i] = previous_score[i * predictor.num_class + j]
                previous_score = new_previous_score
        elif self.previous_score is not None:
            previous_score = np.zeros(self.previous_score.shape, dtype=np.float64)
        else:
            return self
        self.set_previous_score(previous_score)

    def _lazy_init(self, data, label=None, reference=None,
                   weight=None, tau = None, last_pred = None,
                   previous_pred = None, attribute_weight1 = None,
                   attribute_weight2 = None,
                   group=None, init_score=None, previous_score = None,
                   predictor=None,
        ...
        if last_pred is not None:
            self.set_last_pred(last_pred)
        if previous_pred is not None:
            self.set_previous_pred(previous_pred)
        if tau is not None:
            self.set_tau(tau)
        if attribute_weight1 is not None:
            self.set_attribute_weight1(attribute_weight1)
        if attribute_weight2 is not None:
            self.set_attribute_weight2(attribute_weight2)
        ...
        if isinstance(predictor, _InnerPredictor):
            if self._predictor is None and previous_score is not None:
                _log_warning("The previous_score will be overridden by the prediction of init_model.")
            self._set_previous_score_by_predictor(predictor, data)
            ...
            if self.reference is not None:
                ...
                if self.used_indices is None:
                    ...
                    self._lazy_init(self.data, label=self.label, reference=self.reference,
                                    weight=self.weight, tau = self.tau, last_pred=self.last_pred,
                                    previous_pred = self.previous_pred, attribute_weight1=self.attribute_weight1,
                                    attribute_weight2=self.attribute_weight2,group=self.group,
                                    init_score=self.init_score, previous_score= self.previous_score,
                    ...
                    if isinstance(self._predictor, _InnerPredictor) and self._predictor is not self.reference._predictor:
                        ...
                        self._set_previous_score_by_predictor(self._predictor, self.data, used_indices)
            else:
                # create train
    ...
    def create_valid(self, data, label=None, weight=None, tau = None, last_pred=None,
                                    previous_pred = None, attribute_weight1=None,
                                    attribute_weight2=None, group=None,
                     init_score=None, previous_score = None, silent='warn', params=None):
        ...
        ret = Dataset(data, label=label, reference=self,
                      weight=weight, tau = tau, last_pred=last_pred, previous_pred=previous_pred,
                      attribute_weight1=attribute_weight1, attribute_weight2= attribute_weight2, group=group, init_score=init_score, previous_score=previous_score,
    ...
    def save_binary(self, filename):
        ...
        .. note::
            ...
            Please note that `init_score, previous_score` is not saved in binary file.
    ...
    def set_field(self, field_name, data):
        ...
        if data is None:
            ...
            return self
        if field_name == 'init_score' or  field_name == 'previous_score':
            dtype = np.float64
            if _is_1d_collection(data):
                ...
                raise TypeError(
                    'init_score and previous_score must be list, numpy 1-D array or pandas Series.\n'
                    'In multiclass classification init_score and previous_score can also be a list of lists, numpy 2-D array or pandas DataFrame.'
                )
        else:
    ...
    def _set_predictor(self, predictor):
        ...
        if self.handle is None:
            ...
            self._set_previous_score_by_predictor(self._predictor, self.data)
        elif self.used_indices is not None and self.reference is not None and self.reference.data is not None:
            self._predictor = predictor
            self._set_init_score_by_predictor(self._predictor, self.reference.data, self.used_indices)
            self._set_previous_score_by_predictor(self._predictor, self.reference.data, self.used_indices)
        else:
    ...
    def set_last_pred(self, last_pred):
        """Set last_pred of each instance.

        Parameters
        ----------
        last_pred : list, numpy 1-D array, pandas Series or None
            last_pred to be set for each data point.

        Returns
        -------
        self : Dataset
            Dataset with set last_pred.
        """
        # if last_pred is not None and np.all(last_pred == 1):
        #     last_pred = None
        self.last_pred = last_pred
        if self.handle is not None and last_pred is not None:
            last_pred = list_to_1d_numpy(last_pred, name='last_pred')
            self.set_field('last_pred', last_pred)
            self.last_pred = self.get_field('last_pred')  # original values can be modified at cpp side
        return self
    def set_tau(self, tau):
        """Set tau of each instance.

        Parameters
        ----------
        tau : list, numpy 1-D array, pandas Series or None
            tau to be set for each data point.

        Returns
        -------
        self : Dataset
            Dataset with set tau.
        """
        # if tau is not None and np.all(tau == 1):
        #     tau = None
        self.tau = tau
        if self.handle is not None and tau is not None:
            tau = list_to_1d_numpy(tau, name='tau')
            self.set_field('tau', tau)
            self.tau = self.get_field('tau')  # original values can be modified at cpp side
        return self
    def set_attribute_weight1(self, attribute_weight1):
        """Set attribute_weight1 of each instance.

        Parameters
        ----------
        attribute_weight1 : list, numpy 1-D array, pandas Series or None
            attribute_weight1 to be set for each data point.

        Returns
        -------
        self : Dataset
            Dataset with set attribute_weight1.
        """
        # if attribute_weight1 is not None and np.all(attribute_weight1 == 1):
        #     attribute_weight1 = None
        self.attribute_weight1 = attribute_weight1
        if self.handle is not None and attribute_weight1 is not None:
            attribute_weight1 = list_to_1d_numpy(attribute_weight1, name='attribute_weight1')
            self.set_field('attribute_weight1', attribute_weight1)
            self.attribute_weight1 = self.get_field('attribute_weight1')  # original values can be modified at cpp side
        return self
    def set_attribute_weight2(self, attribute_weight2):
        """Set attribute_weight2 of each instance.

        Parameters
        ----------
        attribute_weight2 : list, numpy 1-D array, pandas Series or None
            attribute_weight2 to be set for each data point.

        Returns
        -------
        self : Dataset
            Dataset with set attribute_weight2.
        """
        # if attribute_weight2 is not None and np.all(attribute_weight2 == 1):
        #     attribute_weight2 = None
        self.attribute_weight2 = attribute_weight2
        if self.handle is not None and attribute_weight2 is not None:
            attribute_weight2 = list_to_1d_numpy(attribute_weight2, name='attribute_weight2')
            self.set_field('attribute_weight2', attribute_weight2)
            self.attribute_weight2 = self.get_field('attribute_weight2')  # original values can be modified at cpp side
        return self
    def set_previous_pred(self, previous_pred):
        """Set previous_pred of each instance.

        Parameters
        ----------
        previous_pred : list, numpy 1-D array, pandas Series or None
            previous_pred to be set for each data point.

        Returns
        -------
        self : Dataset
            Dataset with set previous_pred.
        """
        # if previous_pred is not None and np.all(previous_pred == 1):
        #     previous_pred = None
        self.previous_pred = previous_pred
        if self.handle is not None and previous_pred is not None:
            previous_pred = list_to_1d_numpy(previous_pred, name='previous_pred')
            self.set_field('previous_pred', previous_pred)
            self.previous_pred = self.get_field('previous_pred')  # original values can be modified at cpp side
        return self
    ...
    def set_previous_score(self, previous_score):
        ...
        Parameters
        ----------
        previous_score : list, list of lists (for multi-class task), numpy array, pandas Series, pandas DataFrame (for multi-class task), or None
            Init score for Booster.
        ...
        """
        self.previous_score = previous_score
        if self.handle is not None and previous_score is not None:
            self.set_field('previous_score', previous_score)
            self.previous_score = self.get_field('previous_score')  # original values can be modified at cpp side
        return self
    ...
    def get_last_pred(self):
        """Get the last_pred of the Dataset.

        Returns
        -------
        last_pred : numpy array or None
            last_pred for each data point from the Dataset.
        """
        if self.last_pred is None:
            self.last_pred = self.get_field('last_pred')
        return self.last_pred
    def get_previous_pred(self):
        """Get the previous_pred of the Dataset.

        Returns
        -------
        previous_pred : numpy array or None
            previous_pred for each data point from the Dataset.
        """
        if self.previous_pred is None:
            self.previous_pred = self.get_field('previous_pred')
        return self.previous_pred
    def get_tau(self):
        """Get the tau of the Dataset.

        Returns
        -------
        tau : numpy array or None
            tau for each data point from the Dataset.
        """
        if self.tau is None:
            self.tau = self.get_field('tau')
        return self.tau
    def get_attribute_weight1(self):
        """Get the attribute_weight1 of the Dataset.

        Returns
        -------
        attribute_weight1 : numpy array or None
            attribute_weight1 for each data point from the Dataset.
        """
        if self.attribute_weight1 is None:
            self.attribute_weight1 = self.get_field('attribute_weight1')
        return self.attribute_weight1
    def get_attribute_weight2(self):
        """Get the attribute_weight2 of the Dataset.

        Returns
        -------
        attribute_weight2 : numpy array or None
            attribute_weight2 for each data point from the Dataset.
        """
        if self.attribute_weight2 is None:
            self.attribute_weight2 = self.get_field('attribute_weight2')
        return self.attribute_weight2
    ...
    def get_previous_score(self):
        ...
        Returns
        -------
        previous_score : numpy array or None
            Init score of Booster.
        """
        if self.previous_score is None:
            self.previous_score = self.get_field('previous_score')
