''' LGBMFairClassifier for adult fair task (binary classification) Batch testing
/* author: Authorname
'''
import sys 
sys.path.append("..") 
...
for seed in range(39000, 40000):
    ...
    print("**************** Fairlearn-LGBM(EG) Fair Task `adult fair task (binary classification)` ****************")
    version = "0.8.0"
    sensitive_attribute = X_train[:, 9]
    model = lgb.LGBMClassifier(**general_params)
    ...
    ## model info
    csvdata["model"] = "Fairlearn-LGBM(EG)"
    csvdata["time_stamp"] = time_stamp
    csvdata["version"] = version
    csvdata["random_seed"] = seed
    ## save to csv file
